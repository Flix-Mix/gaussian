...
int pthread_setaffinity_np(pthread_t t, size_t c, const cpu_set_t *cpuset);
...
void *FilterCalc(void *threadid)
{
	...
	for (j = tid; j < m; j=j+num_threads)
	{
		for (i = 0; i < m; i++)
		{
			...
			for (bth = up; bth <= down;  bth++)
				for (len = left; len <= right; len++)
...
int main (int argc, char *argv[])
{
	...
	for(t=0; t<num_threads; t++)
	{
		...
		if(affinity){
			cpu_set_t cpu_set;  //create cpu set with core t
			CPU_ZERO(&cpu_set);
			CPU_SET(t+1,&cpu_set);
						//set affinity
			if(!pthread_setaffinity_np((pthread_t) &threadsInt[t], sizeof(cpu_set_t), &cpu_set)){
